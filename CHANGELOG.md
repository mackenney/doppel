...
- CLI `register` accepts `--trailing-run-guard-charset <name>`, for parity with the patterns-file `trailing_run_guard_charset` field. Requires `--trailing-run-guard`; rejected together with `--group`, same as `--trailing-run-guard`.
