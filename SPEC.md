...
The built-in GCP API key pattern SHOULD declare a trailing run guard by default: its structure (a 4-byte fixed prefix followed by a single base64-charset variable region with no trailing anchor) is the statistically weakest in the built-in set against large base64 payloads. When it declares that guard, its guard charset MUST be declared explicitly as `base64_any` — not inferred: the blobs that produce its false positives are standard-alphabet base64, a strict superset of the pattern's `url_safe_base64` match charset, and an inferred (url-safe) guard charset provides no effective suppression against them (see §Trailing Run Guard, Charset resolution). The match charset itself remains `url_safe_base64`; detection precision for genuine keys is unchanged. Other built-in patterns need no guard; their longer prefixes and/or dual anchors make accidental structural matches negligible.
...
- **trailing-run-guard-charset** (charset name, optional, absent by default). When present, the produced Pattern declares this charset as its explicit guard charset (see §Trailing Run Guard, Charset resolution) instead of inheriting the `variable` segment's charset. The value MUST be a valid charset name; an unrecognised name MUST fail with a clear error. Supplying this option without trailing-run-guard MUST fail with a clear error — a guard charset without a guard threshold is meaningless. This option is deliberately not exposed as a CLI `register` flag for now: the match-vs-noise charset divergence that motivates explicit declaration arises in structural family patterns, while registered patterns are HMAC-gated and rarely warrant a guard at all (see trailing-run-guard above), so the CLI's closed `register` option enumeration is left unchanged until a concrete need appears. Callers needing the option today use the library registration API or edit the patterns file directly.
...
27. Given the built-in GCP pattern at its default guard configuration, a structural GCP-shaped candidate embedded in unwrapped standard-alphabet base64 data and followed by at least the default threshold count of standard-base64 bytes is suppressed and produces no entry.
